...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy((x: Char) => x).toList.sorted.map((pair) => (pair._1, pair._2.length))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    List() :: brkup(occurrences).foldLeft(List[Occurrences]())(prod)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def sentenceAnagramsHelfer(senOccr: Occurrences): Set[Sentence] =
      (if (senOccr.isEmpty) List(List())
        ...
        restWords <- sentenceAnagramsHelfer(restOccr)
      } yield wordForComb :: restWords)).toSet

    sentenceAnagramsHelfer(sentenceOccurrences(sentence)).toList
  }
