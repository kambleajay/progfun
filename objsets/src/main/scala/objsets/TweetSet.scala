...
abstract class TweetSet {
  ...
  def ascendingByRetweet0(set: TweetSet, acc: Trending): Trending = {
    	if(set isEmpty) acc 
    	else ascendingByRetweet0(set.remove(set findMin), acc + (set findMin)) 
  }
...
object GoogleVsApple {
  ...
  val allTweetsSet = TweetReader.allTweets

  val googleTweets: TweetSet = allTweetsSet.filter((tw) => google.exists(tw.text.contains(_)))

  val appleTweets: TweetSet = allTweetsSet.filter((tw) => apple.exists(tw.text.contains(_)))
...
object Main extends App {
  ...
  println("RANKED:")
  //println(GoogleVsApple.allTweetsSet foreach println)
  println(GoogleVsApple.googleTweets foreach println)
  println("*********")
  println(GoogleVsApple.appleTweets.foreach(println))
}
